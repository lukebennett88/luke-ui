...
import { extractProseForRules, findProseRuleLabels } from '../src/lib/docs-prose-rules.js';
...
function matchProsePatterns(relativePath: string, source: string): Array<string> {
	const prose = extractProseForRules(source);
	return findProseRuleLabels(prose).map((label) => `${relativePath}: ${label}`);
}
