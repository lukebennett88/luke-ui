...
const codeMessageSchema = z.object({
	type: z.literal('luke-playground:code'),
	code: z.string(),
});

const previewMessageSchema = z.discriminatedUnion('type', [
	z.object({ type: z.literal('luke-playground:ready') }),
	z.object({ type: z.literal('luke-playground:success') }),
	z.object({ type: z.literal('luke-playground:error'), message: z.string() }),
]);
