...
Luke UI defines nine semantic typography styles. Each style is a complete treatment: font family,
size, weight, line height, letter spacing, and Capsize trim.
...
<ExampleBlock src="text/typography" title="Text — Typography styles" />
...
Each typography style already carries its weight. Themes still expose `body`, `label`, `heading`,
and `emphasis` weight roles for overrides and for `Strong`. Text also accepts semantic `color`
