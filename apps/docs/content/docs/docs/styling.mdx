...
`xstyle` replaces a same-property recipe value. One current exception: a `padding` or
`paddingInline` value in `xstyle` does not replace a component's `paddingInlineStart` or
`paddingInlineEnd` — override those with the same logical property instead. This is an upstream
StyleX limitation specific to inline padding. The `margin`, `inset`, `borderWidth`, and
`borderRadius` shorthands all replace their logical longhands as expected. Application styles in a
higher layer, or normal unlayered application CSS, can override `xstyle`. A class authored in a
lower-priority layer such as `base` still loses. For normal declarations, inline `style` wins over
class-based styling. An author `!important` declaration can override a normal inline style.
...
Pass a partial slot map to `xstyle` when only one part needs an override.

```tsx
import * as stylex from '@stylexjs/stylex';
import { inputGroupRecipe } from '@luke-ui/react/primitives/input-group';

const styles = stylex.create({ prefix: { color: 'rebeccapurple' } });
const props = inputGroupRecipe({ xstyle: { prefix: styles.prefix } });
return (
	<div {...props.group}>
		<span {...props.prefix}>Name</span>
	</div>
);
```
...
Without `xstyle`, the same layers apply except `overrides`. Name every layer you use. An omitted
layer is appended after the layers you declare, which changes the override order.

Keep application base styles in `base`, below component recipes. Put overrides in `utilities`, or
use ordinary unlayered application CSS. Tailwind utilities, CSS Modules, and application CSS work
well for application-owned layout and custom elements. Put Tailwind classes on surrounding elements
or `Box`:
