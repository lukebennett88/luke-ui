...
Use a recipe only when the component cannot render the element you need to style. For button-shaped
navigation, prefer [`Link`](/components/actions/link) with `appearance="button"` over applying
`buttonRecipe` to an anchor.
...
The example shows how to apply a component treatment to an owned element:
...
const variants: ButtonRecipeVariants = {
	appearance: 'button',
	tone: 'neutral',
	prominence: 'standard',
};
...
Luke UI's stylesheet uses four cascade layers, from lowest to highest priority: `reset`, `base`,
`recipes`, and `utilities`. That order does not change with stylesheet import order. The reset
applies only under `rootClassName`.

The `base` layer is reserved for application defaults such as Tailwind Preflight. Luke UI declares
it empty so it stays below `recipes`. Put all Luke UI component styles in `recipes`, including
component-owned descendant and combinator selectors. Put one-off overrides in `utilities`.
