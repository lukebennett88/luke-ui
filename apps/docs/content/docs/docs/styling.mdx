...
and inline `style` beats `className`.
...
a `luke` layer that holds its StyleX component styles, `structural`, and `utilities`. That internal
order is stable regardless of stylesheet import order. The reset normalises only the subtree that
carries `rootClassName`.

If your application also uses cascade layers, declare its layer order before you import stylesheets,
and name both `luke` and `structural` in that declaration. Place them below the application layers
that should override Luke UI, such as `components` and `utilities`. You do not need to know how Luke
UI organises styles inside `luke`, only that `luke` is its name. If you omit `luke`, the browser
appends it after your declared layers the first time Luke UI's stylesheet registers it, and Luke
UI's component styles then beat your `components` and `utilities` rules instead of losing to them.

`structural` needs the same treatment, for a different reason. If you omit it, the browser appends
it after your declared layers too, which puts it above `utilities` instead of below it.
`LoadingSkeleton` relies on `structural` sitting below `utilities`, because it uses `!important`
there so its placeholder styles win over wrapped children. Cascade-layer priority reverses for
`!important` declarations, so a lower layer's `!important` rule beats a higher layer's. If
`structural` ends up above `utilities`, a `utilities`-layer `!important` rule from your application
would beat it instead of losing to it.
