...
Prefer `Button` or `Link appearance="button"` first. Reach for `buttonRecipe` only when you must
style an element you do not control, such as a router library's own `Link` component, which takes
router props instead of `href` and cannot be replaced with Luke UI's `Link`.
