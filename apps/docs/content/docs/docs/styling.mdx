...
`xstyle` requires your own StyleX compiler. `@stylexjs/stylex` is already a runtime dependency of
Luke UI, but authoring `stylex.create(...)` needs compilation, and Luke UI's own compiler only
processes its own source. Install `@stylexjs/babel-plugin`, or the StyleX bundler integration your
build already uses, and configure it the way the next section describes. If your application never
uses `xstyle`, it needs no StyleX compiler at all.
...
If you author `xstyle`, add one more layer, `xstyle`, between `recipes` and `components`, and
compile your StyleX styles into it:
...
Configure your StyleX compiler to emit that layer with `useLayers`. With `@stylexjs/babel-plugin`,
pass it to `processStylexRules`:
...
A bundler plugin built on the same compiler exposes the equivalent option under its own name — check
its `useLayers` (or layer-prefix) setting. This is the minimum configuration the published
`xstyle < className` precedence requires. Two other configurations do not give you that guarantee,
so avoid them:
