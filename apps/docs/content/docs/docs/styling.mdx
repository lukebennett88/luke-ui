...
Luke UI's static CSS uses six cascade layers, ordered from lowest to highest priority: `reset`,
`theme`, `base`, `recipes`, `structural`, and `utilities`. That internal order is stable regardless
of stylesheet import order. The reset normalises only the subtree that carries `rootClassName`. The
`base` layer is reserved for application-owned element defaults such as Tailwind Preflight. Luke UI
declares it so it ranks below `recipes`, but never writes to it. Component styles live in `recipes`,
retained global selectors such as skeleton masking live in `structural`, and one-off overrides
belong in `utilities`.
