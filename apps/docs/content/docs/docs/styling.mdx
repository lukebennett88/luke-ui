...
`luke.sx.priorityN`, `structural`, and `utilities`. That internal order is stable regardless of
stylesheet import order. The reset normalises only the subtree that carries `rootClassName`.
