...
nested identity resolves its own radius, space, and type values correctly. Its colour, depth, and
action-control-finish values resolve at equal precedence with the ancestor identity, so stylesheet
