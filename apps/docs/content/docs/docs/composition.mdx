...
For `Box`, use `elementType` for a supported structural element. It does not change the accepted DOM
props. Use `render` when the callback must own the element and its DOM attributes. Do not combine
`render` with `elementType`. Use `Link` and `Button` when you need their behaviour.
