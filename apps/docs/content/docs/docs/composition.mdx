...
Import a recipe from the component or primitive entrypoint that owns the styling contract when the
custom composition follows that structure.

```tsx
import { buttonRecipe } from '@luke-ui/react/button';

const className = buttonRecipe({ appearance: 'subtle', size: 'small' });
