...
The layout properties include background, border, and depth values from the theme scale. Typography
and interaction states stay with a component API. When a custom element that is not a Box needs a
visual token, use `vars` from `@luke-ui/react/theme`. Semantic values follow the active identity and
colour mode, while Luke UI defines its spacing scale and typography styles in source.
