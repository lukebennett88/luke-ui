...
Padding, gap, and margin accept `0` plus the value-based spacing keys `sp4`, `sp8`, `sp12`, `sp16`,
`sp24`, `sp32`, `sp40`, `sp48`, and `sp64`. Each spacing key matches its pixel value, so `sp16` is
16px. Margin also accepts `auto`. Sizing and grid-placement properties accept their CSS values.
