---
title: Layout
description: Build responsive structure with Box, layout utilities, and width-keyed breakpoints.
---
...
constant, or an object keyed by breakpoint when it changes at a given container inline size.
...
set at `640` also applies at `768` and above, unless a later breakpoint overrides it. Values cascade
up from `initial`, so specify only the points where the layout changes.

```tsx
<Box display="flex" flexDirection={{ initial: 'column', '768': 'row' }}>
	{children}
</Box>
```

This example stacks the children by default, then places them in a row from `768`.
