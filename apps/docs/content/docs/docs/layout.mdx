...
Import `breakpoints` from `@luke-ui/react/styles` when you author your own `@container` queries so
they use the same thresholds as Luke UI responsive props:

```tsx
import { breakpoints } from '@luke-ui/react/styles';

const wideEnough = `(inline-size >= ${breakpoints.bp768}px)`;
```

Use that condition in a consumer-authored `@container` rule. For example, in Vanilla Extract:
...
export const sidebar = style({
	'@container': {
		[`(inline-size >= ${breakpoints.bp768}px)`]: {
			display: 'grid',
		},
	},
});
