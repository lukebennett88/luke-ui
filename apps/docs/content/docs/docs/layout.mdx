...
<ExampleBlock src="layout/breakpoints" title="Layout: Shared breakpoints" />
...
number in each key names its step on the scale, so `sp24` is one step larger than `sp16`. Margin
also accepts `auto`. Sizing and grid-placement properties accept their CSS values.
