...
number in each key names its pixel-equivalent design step, which emits as `rem`, so `sp16` is `1rem`
and `sp24` is one step larger. Margin also accepts `auto`. Sizing and grid-placement properties
