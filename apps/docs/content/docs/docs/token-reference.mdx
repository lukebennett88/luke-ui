...
description: Public CSS variables to build custom Luke UI elements.
---
...
Spacing provides one scale for `gap`, `padding`, and `margin`. The number in each key names its
pixel-equivalent design step and emits as `rem`, so `sp16` is `1rem` and `sp24` is one step larger.
