...
Choose a token by the job it performs. Spacing uses `sp`-prefixed pixel sizes. Radius and depth use
semantic roles. Use the public variable instead of copying its resolved CSS value.
