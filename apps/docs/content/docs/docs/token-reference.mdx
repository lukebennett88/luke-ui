...
variable. For example, `vars.color.background.danger.solid.rest` resolves to
`var(--luke-color-background-danger-solid-rest)`.
...
Pair each depth role with the matching surface role. A card might use `surface.floating` with
`depth.floating`, while a dialog can use `surface.overlay` with `depth.overlay`.
