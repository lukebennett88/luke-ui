...
Pair each depth role with the matching surface role. Menus, dialogs, popovers, and sheets use
`color.surface.floating`. Pair a dialog with `depth.overlay` when it needs more lift than a menu.
`color.overlay.backdrop` is the translucent layer behind that dialog. It is not a surface colour and
not a shadow.
