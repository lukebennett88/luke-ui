...
Create a separate theme root for an independent subtree that needs another identity.

## Choose a colour mode

Omit `data-color-mode`. The browser's `prefers-color-scheme` setting then selects the mode. Set
`data-color-mode="light"` or `data-color-mode="dark"` when a root or nested scope needs a fixed
mode.
...
Luke UI's Combobox does this for its popover. It finds the active identity and nearest explicit
colour mode around the trigger, then applies them to the popover root. Application-owned portals
must preserve the same context.
