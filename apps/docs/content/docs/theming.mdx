...
Each identity includes light and dark colour values. An explicit `data-color-mode` is optional;
without one the theme follows the system preference. Colour-mode scopes can nest within an identity.
Identity classes cannot.
