...
size, place an item in a flex or grid layout, or hide content at a breakpoint.
...
Token and enum properties accept a direct value or an object keyed by breakpoint. Values cascade
from `xsmall`, so specify only the points where the layout changes. Arbitrary sizing, inset, flex,
and grid values are intentionally direct-only; use custom CSS when those values need breakpoints.
...
their CSS values as non-responsive escape hatches.
...
The layout utilities exclude semantic colour, typography, and interaction states. Choose a component
API for those concerns. When a custom element needs a visual token, use `vars` from
`@luke-ui/react/theme` rather than a palette value or component selector.
