...
its size the same way. Place an item in a flex or grid layout, or hide content at a breakpoint.
...
| Guidance | Practice                                                                                                   |
| -------- | ---------------------------------------------------------------------------------------------------------- |
| Do       | Use `Box` when an element needs layout properties. Use a plain `div` when it needs none.                   |
| Do       | Give a property an object keyed by breakpoint, and let the value cascade up from `xsmall`.                 |
| Don't    | Expect Sprinkles to set semantic colour, typography, or interaction states. Use a component API or `vars`. |
| Don't    | Write a pixel value where a spacing step exists. The steps come from the active theme.                     |
