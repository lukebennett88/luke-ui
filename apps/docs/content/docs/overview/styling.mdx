...
that run Panda can opt into `@luke-ui/react/preset` for the shared layer order. Keep the starter as
the first statement in global CSS and extend it with any consumer-owned layer names.
...
The preset does not replace the prebuilt stylesheet or theme CSS: import both before using its token
aliases. It intentionally exports neither a prebuilt `css()` helper nor a separate token entrypoint.
