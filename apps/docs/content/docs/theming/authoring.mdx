...
- `color.neutral` or `color.neutralStyle` (`'cool'`, `'neutral'`, or `'warm'`) for the neutral
  canvas anchor
- optional `color.background` to split the page canvas from the neutral family's hue and chroma
...
a WCAG 2.2 validation matrix over the result. Not every pair in that matrix can fail a build — some
are guaranteed, some are only measured, and one category is not checked at all.
...
built specifically to clear this boundary — it is not a scale-step alias that happens to pass.
...
reported, never thrown. These borders come from the same subtle scale step as ordinary separators,
by design, so an author can choose a neutral character whose intent borders read as gentle
...
**An advisory border must never be the only cue for a state.** Because these pairs are not
guaranteed to reach 3:1, do not rely on an intent border alone to communicate an error, warning, or
success state — pair it with text, an icon, or a guaranteed-contrast surface change so the state is
still legible for a theme whose intent border happens to measure under 3:1.
...
solid colour. `defineTheme` adapts a single-value accent by holding its hue and chroma and searching
for a lightness that clears the same on-solid gate the generator enforces internally, and that
adaptation is forgiving in practice. An explicit `{ light, dark }` accent skips that adaptation and
is used verbatim for each side — if its authored lightness sits in an on-solid dead zone (no
near-white or near-black text reaches 4.5:1 against it), `compileTheme` throws
`ThemeGenerationError` naming the failing role and mode. Author an explicit `{ light, dark }` accent
when you need exact control over its lightness; author a single value and let `defineTheme` adapt it
when you do not.
