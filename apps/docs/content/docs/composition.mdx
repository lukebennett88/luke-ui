...
the task. It handles the intended behaviour, accessibility, and visual treatment together, so use
its supported props before you build a custom pattern. Compose from a documented primitive only when
a component's props do not fit.
