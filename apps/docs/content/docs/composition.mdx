...
Choose between `Box`'s two related props for the same reason. Set `elementType` when you only need a
different supported element, such as an `a` instead of a `div`. Use `render` to wrap the element in
a compatible custom component, such as a motion wrapper. It keeps the element a `div`.

<SourceCodeBlock src="composition/box-element-choice" />
