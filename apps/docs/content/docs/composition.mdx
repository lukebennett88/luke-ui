...
For `Box`, set `elementType` when the layout needs a supported structural element. This choice does
not change Box's accepted DOM props. Box owns the rendered element.

<ExampleBlock src="box/semantic-element" title="Composition and customisation — Semantic element" />

Use Box's `render` prop when the callback must own the rendered element and its DOM attributes. The
callback can return an appropriate intrinsic element or custom component. Do not combine `render`
with `elementType`. Use `Link` and `Button` when you need their behaviour.
