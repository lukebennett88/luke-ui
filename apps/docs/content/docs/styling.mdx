...
point. Render `Theme` with `name` set to the identity's name at the application or subtree root.
Luke UI scopes the reset to that root, so it does not reset unrelated application content.
...
order. The reset normalises only the subtree under a `Theme` root.
