...
Use `tone` to draw attention to a destination. `neutral` is the default. Choose `accent` for a
destination that needs more emphasis.
...
on hover and press. Use `high` with the `accent` tone for a primary destination.
