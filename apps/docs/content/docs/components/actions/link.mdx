...
`standard` is the default prominence. Choose `low` for quiet navigation items. Its underline appears
on hover, focus, and press. Choose `high` for a primary destination. The theme uses the primary
accent treatment for high prominence.
...
Replace `isStandalone` with `prominence="low"`. Links now use neutral text at standard prominence,
and high prominence selects the primary treatment. Set `appearance="button"` when navigation needs a
button-shaped call to action.
