...
`neutral` is the default tone for ordinary actions. Set `tone="critical"` for a destructive action.
Prominence sets hierarchy. The theme uses the primary accent treatment for high prominence.

<ExampleBlock src="button/tones" title="Button — Tone" />

## Prominence

Use `prominence` to set visual weight. `standard` is the default. Choose `low` for quiet secondary
actions and `high` for primary or destructive confirmation actions. Text Buttons support neutral
low, standard, and high prominence. Critical text Buttons support low and standard prominence.
...
Replace `appearance="ghost"` with `prominence="low"`, `subtle` with `standard`, and `solid` with
`high`. Replace `tone="danger"` with `tone="critical"`. `accent` is no longer a consumer-selectable
tone. Keep `tone="neutral"` where you name the ordinary action tone.
