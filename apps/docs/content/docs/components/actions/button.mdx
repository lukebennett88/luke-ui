...
and `critical` marks a destructive one. The supported tone and prominence pairs are curated by
appearance, so unsupported combinations are rejected by TypeScript.
...
actions and `high` for primary or destructive confirmation actions.
