...
`onPress` handles the synchronous interaction. `pressAction` performs Button-owned async work whose
pending lifecycle the button manages automatically. When both are set, `onPress` runs first, then
`pressAction`.

Prefer a native form Action when the work is naturally a form submission:
...
Pending buttons stay focusable and ignore further presses. `pressAction` creates that pending state
automatically. Explicit `isPending` shows a spinner immediately. An Action-owned spinner waits
briefly so a fast Action never flashes one. Update the visible label when the status matters, for
example from “Save” to “Saving”.
