...
`IconButton` shares Button's appearance API. Tone can be `neutral`, `accent` (default), or `danger`.
Variant can be `solid` (default), `subtle`, or `ghost`.

<ExampleBlock
	src="icon-button/tones"
	title="Icon Button — Tone and variant"
	description="Semantic tones and emphasis variants for icon-only actions."
/>
