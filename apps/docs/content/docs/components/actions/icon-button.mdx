...
`pressAction` for IconButton-owned async work, and `isPending` for externally owned pending state.
Pending controls stay focusable, ignore further presses, and show the same spinner timing as
`Button`.
