...
Pass `xstyle` to style a property `VisuallyHidden`'s own styles do not set, as one or more
`stylex.create(...)` style objects. It applies after `VisuallyHidden`'s own styles and before
