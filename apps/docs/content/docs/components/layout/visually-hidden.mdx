...
Use [`IconButton`](/components/actions/icon-button) or [`IconLink`](/components/actions/icon-link)
for an icon-only control instead of hiding label text yourself. Their required accessible name
