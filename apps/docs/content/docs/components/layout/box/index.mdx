...
Box's accepted DOM props. Without `render`, Box owns this element and its common DOM attributes.
...
an appropriate intrinsic element or custom component. It receives the resolved children, generated
and consumer styling, and a neutral callback ref.

In this mode, Box accepts layout props plus `children`, `className`, `style`, and `ref`. Put event,
accessibility, and element-specific attributes on the element inside the callback.
