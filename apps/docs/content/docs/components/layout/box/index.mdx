...
`Box` is a `div` that accepts Luke UI layout properties. It places content in a layout, adds token
spacing, constrains an element's size, or controls a flex or grid child. Use a normal `div` when no
supported layout property applies.
...
See [Breakpoints](/overview/breakpoints) for the available keys and fixed thresholds.
...
and event handlers. `render` does not make Box polymorphic. It must return a `div`.
