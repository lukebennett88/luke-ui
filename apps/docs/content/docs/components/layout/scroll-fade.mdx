...
Use [`Box`](/components/layout/box) with `overflow` set directly for a plain scroll container that
does not need the fade or the focusable, named overflow region.
