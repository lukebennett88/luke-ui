...
gap, flex, and grid-child properties. It does not set grid container properties. Use
[Grid](/components/layout/grid) for an explicit equal column count, or
[AutoGrid](/components/layout/auto-grid) when columns should grow from a minimum inline size.
...
Use [Stack](/components/layout/stack) for a vertical column and
[Cluster](/components/layout/cluster) for a wrapping inline group. Both set Box's flex properties
for you.
