---
title: Aspect ratio
description: Lock media to an inline-to-block ratio.
source: packages/@luke-ui/react/src/exports/aspect-ratio.ts
---

`AspectRatio` locks a frame to an inline-to-block ratio and sizes its direct child to fill the
frame. Use it for media such as an `iframe`, `img`, or `video`. Images and videos use
`objectFit="cover"` by default. Pass another `objectFit` value to change how they fit the frame.
Read [Layout](/docs/layout#choose-a-layout-component) to choose a layout component.

<ExampleBlock src="aspect-ratio/basic" title="Aspect ratio — Basic" />

## Choose the rendered element

AspectRatio uses
[Box's `elementType` and `render` contract](/components/layout/box#choose-the-rendered-element).

<ExampleBlock src="aspect-ratio/semantic-element" title="Aspect ratio — Figure" />

## Related components

Use [`Box`](/components/layout/box) for non-media content, such as a coloured panel or a card, that
needs a frame but not a locked ratio between its dimensions.

## API

<component-props-table
	path="packages/@luke-ui/react/src/core/aspect-ratio/aspect-ratio.tsx"
	name="AspectRatioProps"
/>
