...
[`Button`](/components/actions/button) has its own `isPending` and `pressAction` props for a
standard loading action. For a labelled button that keeps its text visible, wrap the icon in
`LoadingSpinner`. Disable the button while the action runs.
