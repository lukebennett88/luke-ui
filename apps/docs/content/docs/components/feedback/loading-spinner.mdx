...
Pass `xstyle` with one or more `stylex.create(...)` style objects for styles `LoadingSpinner`'s own
...
over a variant such as `color`. `className` still beats `xstyle`. `xstyle` applies to the spinner
itself, not the in-place content wrapper.
