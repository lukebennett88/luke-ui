...
On a mobile device, the field shows a button-like control with the selected option. When someone
uses the control, a modal tray opens with a focused search input. The tray has a fixed height and
locks page scroll.
