...
An invalid field switches to a 2px boundary and shows an error icon after the clear and trigger
buttons. The icon is a non-colour cue, so an invalid field stays perceivable even when
