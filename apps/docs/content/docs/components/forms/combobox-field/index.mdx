...
An invalid field switches to a 2px boundary and shows a filled error badge after the clear and
trigger buttons. The badge is a non-colour cue, so an invalid field stays perceivable even when
