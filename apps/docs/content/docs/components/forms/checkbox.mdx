...
Pass `xstyle` with one or more `stylex.create(...)` style objects for styles `Checkbox`'s own props
do not cover, or to override a same-property variant. Styles apply in this order: variant props,
then `xstyle`, then `className`, then inline `style`. A same-property `xstyle` value wins over a
variant such as `size`. `className` still beats `xstyle`.
