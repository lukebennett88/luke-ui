...
option whose child options are only partly selected. `isSelected` and `isIndeterminate` are
independent: a parent checkbox can be selected and indeterminate at once, such as while its children
are still being fetched, and the indeterminate mark takes precedence over the checkmark.
