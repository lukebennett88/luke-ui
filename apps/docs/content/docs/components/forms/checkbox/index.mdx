...
An invalid checkbox switches its control to a 2px boundary and shows a filled error badge after the
label. The badge is a non-colour cue, so an invalid checkbox stays perceivable even when
