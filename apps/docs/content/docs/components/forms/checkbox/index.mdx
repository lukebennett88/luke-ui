...
An invalid checkbox switches its control to a 2px boundary and shows an error icon after the label.
The icon is a non-colour cue, so an invalid checkbox stays perceivable even when `errorMessage` is
omitted.
