...
An invalid field shows an error icon after the input value. The icon is a non-colour cue, so an
invalid field stays perceivable even when `errorMessage` is omitted.
