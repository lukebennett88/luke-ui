...
An invalid field switches to a 2px boundary and shows a filled error badge after the input value.
The badge is a non-colour cue, so an invalid field stays perceivable even when `errorMessage` is
omitted.
