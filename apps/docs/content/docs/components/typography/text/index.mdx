...
choose a semantic element without changing the type treatment. Capsize trim is applied to known
block text elements such as `p`, `div`, headings, and `blockquote`, and skipped automatically for
inline or unknown element types.
