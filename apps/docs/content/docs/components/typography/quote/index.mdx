---
title: Quote
description: Short inline quotation.
---

`Quote` marks a short quotation inside a sentence. It renders a native `q` element, which conveys quotation semantics to assistive technology and receives the browser's own quotation marks.

<ExampleBlock
	src="quote/basic"
	title="Quote — Basic"
	description="A short quotation within a paragraph."
/>

`Quote` stays inline and inherits the surrounding typography. Use `textWrap` to control wrapping, `lineClamp` to constrain a longer quotation, and `cite` to give the source URL.

## Related components

Use [Blockquote](/components/typography/blockquote) for a block-length quotation.
