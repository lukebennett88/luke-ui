...
props:
  - name: HeadingProps
    path: packages/@luke-ui/react/src/heading/index.tsx
---
...
the `heading` font-weight role. Set `size` to change the visual type style without changing the
semantic level. Use `display` for oversized marketing-style headings. Set `color` for a semantic
text colour, such as `info` for a heading that needs to read as informational.

<ExampleBlock src="heading/typography" title="Heading — Type style and colour" />
