...
`Code`, `Em`, `Kbd`, and `Strong` build on `Text` and keep the transform and numeric glyph style of
the text around them, so a word marked up inside uppercase or tabular-numeric text still matches it.
Set `textTransform` or `fontVariantNumeric` on one of them to override what it inherits.
