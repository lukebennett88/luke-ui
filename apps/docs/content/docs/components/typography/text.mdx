---
title: Text
description: Styled text with composite typography and semantic color roles.
---

`Text` expects the Luke UI theme class at the app or root level. See
[Getting Started](/docs/getting-started).

Use `Text` for styled text that should not create heading semantics.

```tsx
<Text>The quick brown fox jumps over the lazy dog.</Text>
```

## Best practices

| Guidance | Practices                                                                                       |
| -------- | ----------------------------------------------------------------------------------------------- |
| Do       | Choose one composite `size` step so font size, line height, spacing, and trim stay coordinated. |
| Don't    | Use `Text` for section headings. Use `Heading`, which manages semantic level automatically.     |

## Typography

<ExampleBlock
	src="text/typography"
	title="Text — Typography"
	description="Use composite size steps, semantic colors, and font-weight roles to style text."
/>

The active theme supplies the curated font family and the weights behind each role. `Text` trims the
space above capitals and below the baseline by default. Set `shouldDisableTrim` when the text must
participate in an untrimmed layout; line clamping disables trim automatically.

## Text transform and decoration

<ExampleBlock
	src="text/transform"
	title="Text — Transform and decoration"
	description="Apply text transform and decoration to emphasize content."
/>

## Truncation

<ExampleBlock
	src="text/truncation"
	title="Text — Truncation"
	description="Clamp lines, truncate to one line, or disable cap-height trim."
/>

## Alignment and numeric glyphs

<ExampleBlock
	src="text/alignment"
	title="Text — Alignment and numeric glyphs"
	description="Align text and set numeric glyph styles such as tabular numerals."
/>

## Token reference

### `color` tokens

`primary`, `secondary`, `accent`, `info`, `success`, `warning`, `danger`

### `size` steps

`100`, `200`, `300`, `400`, `500`, `600`, `700`, `800`, `900`

`300` is the default. Each step combines font size, line height, letter spacing, and Capsize trim.

### `fontWeight` roles

`body`, `label`, `heading`, `emphasis`

### `textDecoration` values

`none`, `underline`, `line-through`, `inherit`

### `textTransform` values

`none`, `capitalize`, `uppercase`, `lowercase`, `inherit`

## When to use vs Heading

Use `Heading` for actual section headings because it manages semantic level nesting automatically.

Use `Text` with a larger `size` and the `heading` or `emphasis` `fontWeight` role when content
should look prominent but is not semantically a heading, such as a large stat number.

## Props

<auto-type-table path="packages/@luke-ui/react/src/text/index.tsx" name="TextProps" />
