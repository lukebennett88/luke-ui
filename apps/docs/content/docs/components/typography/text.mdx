...
<ExampleBlock src="text/lead" title="Text — Lead typography" />
...
Pass `xstyle` to style a property `Text`'s own props do not expose, as one or more
`stylex.create(...)` style objects. It applies after every prop above and before `className`, so a
consumer `className` still beats it. It is not a reliable way to override a property a prop such as
`color` or `typography` already sets — use that prop instead.
...
<component-props-table path="packages/@luke-ui/react/src/core/text/text.tsx" name="TextProps" />
