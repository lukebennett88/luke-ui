---
title: Text
description: Styled text with semantic type styles and colour roles.
...
Use `size` to apply a complete type style: family, size, weight, line height, letter spacing, and
Capsize trim. `body` is the default.
...
you. Use `Text` with a larger type style such as `display` or `heading1` for prominent content that
is not a heading. A large status value is one example.
