...
The primitive renders a single `<button>` element by default. It applies Luke UI size, tone, and
block styles, then renders children directly.

`isPending` disables the button, but it does not add a spinner or hide the label. Build that UI in
children when you use the primitive.
