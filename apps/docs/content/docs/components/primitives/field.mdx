...
Pass `xstyle` with one or more `stylex.create(...)` style objects for styles a field part's own
...
over a variant such as `necessityIndicator`. `className` still beats `xstyle`.
