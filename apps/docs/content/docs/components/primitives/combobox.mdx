...
`ComboboxTray` presents options in a full-screen tray. Pair it with `ComboboxTrayTrigger`, which
shows the current selection and opens the tray.

<ExampleBlock src="combobox-primitive/tray" title="Combobox primitives — Tray" />

Inside the tray, `ComboboxInput` is the search field and `ComboboxClearButton` clears both the
search and the selection. `ComboboxField` chooses between the tray and the popover for you.

The tray holds the only text input, so `ComboboxTrayTrigger` keeps native form validation available
while the tray stays closed. It reads `isRequired`, `validationBehavior`, `isDisabled`,
`isReadOnly`, `form`, and `allowsCustomValue` from the root and applies the same constraint
validation those props describe when the input is visible. A required combobox blocks submission
until someone chooses a value, or, with `allowsCustomValue`, enters custom text. Set
`validationBehavior="aria"` on the root to report validation through ARIA instead.

`ComboboxTrayTrigger` also keeps submitting the root's `name` while the tray stays closed. By
default the combobox submits the selected key. Set `formValue="text"`, or set `allowsCustomValue`,
to submit the text instead. With `allowsCustomValue`, typed text that matches no option has no key
to submit. In text mode a selected option submits its text rather than its key, and the search field
itself submits nothing, so a field submits exactly one value whether the tray is open or closed. A
read-only or `validationBehavior="aria"` combobox still submits its value. In text mode a disabled
combobox submits nothing, but in key mode React Aria's own hidden input keeps submitting the
selected key even when the combobox is disabled.
