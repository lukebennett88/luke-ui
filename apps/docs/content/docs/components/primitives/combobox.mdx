...
`ComboboxLoadMoreItem` components can override that size when necessary. `ComboboxInputGroup` sizes a
nested `Icon` the same way `InputGroup` does at the same control size.
