...
Pass `xstyle` with one or more `stylex.create(...)` style objects for styles a combobox part's own
props do not cover. Styles apply in this order: the part's own styles, then its variant props, then
`xstyle`, then `className`, then inline `style`. A same-property `xstyle` value wins over a variant
such as `size`. `className` still beats `xstyle`.
