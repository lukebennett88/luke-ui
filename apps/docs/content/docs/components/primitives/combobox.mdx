...
takes focus as the tray opens. `ComboboxClearButton` clears the search field and hides while the
search is empty. Because the search field is the only way to choose an option in the tray, clearing
it also clears the selection. `ComboboxListBox` fills the tray and contains its scrolling. Arrow
keys move through options while focus stays in the search field.
