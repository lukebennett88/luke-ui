...
<ExampleBlock
	src="combobox-primitive/basic"
	title="Combobox primitives — Basic"
	description="The lower-level parts composed into a searchable select."
/>

## Anatomy

`ComboboxRoot` owns the searchable single-select state. Put the control and popover inside it. Put
the text input and trigger in `ComboboxInputGroup`, then put the listbox and its items in
`ComboboxPopover`.
...
`ComboboxInput` opens the menu when clicked and supplies the editable combobox input.
...
`ComboboxLoadMoreItem` components can override that size when needed.
