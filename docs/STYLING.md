...
`elementType` to render a different element from a curated set of layout and sectioning elements.
Use `render` to wrap a compatible custom `div` while preserving the generated class, style, ref, and
DOM props. The two props cannot be combined. `Box` provides no `asChild` style polymorphism.
