...
Icon sizes run from `xsmall` to `large` at 16, 20, 24, and 32 pixel-equivalent design units (`1rem`,
`1.25rem`, `1.5rem`, and `2rem`). `ICON_SIZE_VALUES` in `src/theme/token-values.ts` sets their
dimensions.
