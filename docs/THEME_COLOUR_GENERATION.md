...
3. Derives the mode-aware elevation surfaces (`canvas`/`recessed`/`floating`) with
...
5. Runs the full WCAG 2.2 validation matrix (`validateContrast`), which stays authoritative and
   throws `ThemeContrastError` on a hard-gate miss. Interaction checks share
   `mixInteractionColor` with the recipes.
...
actually reference (`canvas`, `recessed`, `floating`), which changes the contrast matrix
...
choose clears the AA text ratio plus the search headroom against the public resting solid.
Hover and pressed colours are derived from that fill by `interactionColor`, plus depth, finish, and
transform. Those derived fills are hard-gated by `validateContrast`.
...
Tokens describe semantic colours. Transient hover and pressed colours are derived from them.

`interactionColor(base, 'hover' | 'pressed')` owns the complete algorithm:
...
those resting colours rather than replacing them.
