...
3. Derives the mode-aware elevation surfaces (`canvas`/`recessed`/`floating`) with `elevation.ts`'s
   `generateSurfaces`. `surfaces.canvas` is always exactly the resolved `background` — canvas IS the
   background, not a derived value.
...
actually reference (`canvas`, `recessed`, `floating`), which changes the contrast matrix from the
pre-v2 solver's.
...
pressed colours are derived from that fill by `interactionColor`, plus depth, finish, and transform.
Those derived fills are hard-gated by `validateContrast`.
