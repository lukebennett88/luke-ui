...
- Hover and pressed mix `color.overlay.tint` into the current semantic fill, so a custom control
  should not pick a different background token to mean hovered or pressed.
