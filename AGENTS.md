...
- Follow the code-style rules in [docs/CONVENTIONS.md#code-style](docs/CONVENTIONS.md#code-style)
  for control flow, collections, and regex.

## Dev loop

- Run `pnpm run check` from the repo root before committing. `check` covers barrels, format, lint,
  types, and docs but **not** tests. Run `pnpm run test` separately, or rely on the `storybook.yml`
  CI workflow to run unit, browser, and docs tests on pull requests.
