...
- A package-local `check:*` run is not what CI runs. Only the root `pnpm run check` runs
  ...
  `pnpm run fix:format-root`. Do not re-wrap the text by hand.
- In a new worktree, run `pnpm run build:packages` before `pnpm run test`. The `test` task depends
  on `^build`, which does not build the own `dist/` of `@luke-ui/react`, and some unit tests read
  `dist/stylesheet.css`.
- The Turbo cache is shared across worktrees, so `FULL TURBO` can replay the result of another
  branch. For a real run, use `TURBO_FORCE=true pnpm run test`. Do not use
  `pnpm run test -- --force`, because the flag goes to the package script.
...
- Lint rules live in the `lint` block of the root `vite.config.ts`. A `.oxlintrc.json` file is
  ignored, and oxlint has no `no-restricted-syntax` rule. Two `lint.overrides` entries that match
  the same file and set the same rule do not merge: the later entry wins.
- In `apps/docs`, a `src/lib/` module that a client component imports must not import `node:fs` or
