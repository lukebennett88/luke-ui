...
export function assembleStylesheet(
	...
	return [
		...
		'',
		// Root-only containment: responsive sprinkles resolve against this container, so it must sit
		// only on the true document root. It is deliberately excluded from the identity-class rules
		// below, because every theme-scoped element and body-level portal (the combobox popover, the
		// mobile tray) also carries the identity class — sizing a container there would make responsive
		// props resolve against a popover's own narrow width instead of the page.
		':where(:root) {',
