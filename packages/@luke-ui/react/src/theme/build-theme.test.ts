...
// against. Every generated colour, depth, and identity value below must stay byte-identical to
// today; only the generator-version header line is expected to differ (see below) and is excluded
// from the comparison.
describe('compatibility (pre-v2) goldens', () => {
	...
	// The goldens were frozen from the literal `buildTheme` output before `THEME_GENERATOR_VERSION`
	// existed, so they carry the old, un-versioned header comment verbatim. That header is generator
	// metadata, not token output, and is *expected* to move when the generator version changes — so
	// the assertions below drop the first line from both sides before comparing. Everything else
	// (every colour, depth, identity, and font value) must remain byte-for-byte identical to the
	// pre-v2 baseline; only the header line is normalised out.
	function dropHeaderLine(css: string): string {
		const newlineIndex = css.indexOf('\n');
		if (newlineIndex === -1) throw new Error('expected the stylesheet to contain a header line');
		return css.slice(newlineIndex + 1);
	}
	...
	it('keeps every generated token byte-identical to the pre-v2 baseline', async () => {
		...
		expect(dropHeaderLine(currentTactile)).toBe(dropHeaderLine(goldenTactile));
		expect(dropHeaderLine(currentPaper)).toBe(dropHeaderLine(goldenPaper));
	});

	it('emits the versioned generator header without disturbing any other line', async () => {
		const goldenTactile = await readFile(compatGoldens.tactile, 'utf8');
		const currentTactile = buildTheme(tactileFoundation);
		...
		expect(currentTactile).toContain(
			`/* Generated by @luke-ui/react theme generator v${THEME_GENERATOR_VERSION}. Do not edit. */`,
		);
	});
});
