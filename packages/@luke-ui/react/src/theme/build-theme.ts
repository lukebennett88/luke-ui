...
/**
 * The private generator's version. A private generator change repaints every emitted colour, so it
 * is not a semver-visible change on its own, but it is not "zero behaviour change" either — bumping
 * this is how a generator revision (e.g. the Stage 6 family-and-mapping engine) becomes discoverable
 * in diagnostics, snapshots, and the generated stylesheet header without touching the public
 * contract. Not exposed publicly unless a real consumer need emerges.
 */
export const THEME_GENERATOR_VERSION = 2;
