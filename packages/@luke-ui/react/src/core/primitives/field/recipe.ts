	...
	variants: {
		...
		tone: {
			...
			error: {
				message: {
					color: vars.color.foreground.danger.rest,
					// Hanging indent, not `flex`: `errorMessage` is typed `ReactNode` (rich content) and
					// RAC's `FieldError` also accepts a render-prop child, so this recipe cannot wrap the
					// message in a span of its own. `padding-inline-start` reserves
					// `--luke-field-message-indent` (`0px` unless a consumer sets it, e.g. `Checkbox`
					// aligning its message under its label) on every line, then `textIndent` pulls the
					// first line back by that same amount so the icon sits in it. Wrapped lines keep the
					// padding, so they hang aligned with the text rather than tucking under the icon.
					paddingInlineStart: 'var(--luke-field-message-indent, 0px)',
